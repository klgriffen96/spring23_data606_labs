...
In general, the number of girls baptized over the years is increasing linearly. However, there is an exception where there is a decrease in baptisms from 1640-1650 and the baptisms performed remains low from 1650 - 1660 before going back to increasing.The plots backing up this analysis are displayed above.
...
Make a new variable for the total number of baptisms and then use it to make another variable by computing the boy-to-girl ratio. 
```{r calculate-prop-boys-arbuthnot}
arbuthnot <- arbuthnot %>%
  mutate(total = boys + girls)
...
arbuthnot <- arbuthnot %>%
  mutate(boy_ratio = boys / total)
```
Next, plot the proportion of boys born over time.
...
The proportion of boys born over time in this dataset is generally between 0.51 and 0.53.
...
Dimensions are 63 cases and 3 variables (year, boys, girls).

5.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?
```{r counts-arbuthnot-vs-present}


(mean(present$boys) + mean(present$girls))/ (mean(arbuthnot$boys) + mean(arbuthnot$girls))
```
Arbuthnot's data set consisted of 82 years of data, from 1629 - 1710 while the present dataset consisted of 63 years of data from 1940-2002. The number of births of boys and girls was significantly larger (about 320x larger) in the present dataset vs the Arbuthnot datset.
...
Make a new variable for the total number of births and then use it to make another variable by computing the boy-to-girl ratio. 
```{r calculate-prop-boys-present}
present <- present %>%
  mutate(total = boys + girls)
...
Based on the plot in the U.s.for the given dataset, boys are born in a greater propotion than girls.
...
The most total births in the U.S. according to this dataset was in 1961.
