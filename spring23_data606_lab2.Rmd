...
The histogram with a binwidth of 15 has the finest resolution and the histogram with a binwidth of 150 is most coarse. The histogram with a binwidth of 15 shows a feature that the others do not, a shorter bar to the left of the largest bin near dep_delay =  0. The histogram with a bindwidth of 15 is also the smoothest histogram, where the other two display greater jumps between the bins.
...
```{r sfo-feb-flights-2}
# Create the dataframe as done above
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO", month == 2)
# Count the flight
sfo_feb_flights %>% 
  summarise(n = n())
```
In February, 68 flights headed to SFO.   
...
```{r arrival-delay-dist-10}
# Plot the histogram of arrival delays of SFO Feb
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) +
  geom_histogram(binwidth = 10)
```
    
After experimenting with different binwidths, binwidth = 10 was found to be a good choice for this dataset. The dataset appears to follow a mostly normal distribution, but you might say that the distribution is slightly skewed to the right (where there are some outliers).

For this case, due to the right skew - the median and the IQR are better summary statistics to look at because they are more robust to outliers and skewness than the mean and the standard deviation. I added the count as well just to get an idea of how many data points there are.

```{r arrival-delay-dist}
...
```{r summary-arr_delay-SFO-feb}
arr_delay_carrier <- sfo_feb_flights %>%
                    group_by(carrier) %>%
                    summarise(median_ad = median(arr_delay), 
                        iqr_ad = IQR(arr_delay), 
                        n_flights = n()) %>%
                    arrange(desc(iqr_ad))
```
The carriers which have the most variable arrival delays are DL and UA.
...
dep_delay_month <- nycflights %>%
                  group_by(month) %>%
                  summarise(mean_dd = mean(dep_delay)) %>%
                  arrange(desc(mean_dd))
head(dep_delay_month)
```

Based on this, July has the greatest average departure delays from NYC.
...
Based on the histograms from the initial exercises of departure delays, the data does not appear symmetrically distributed. When the data is symmetric, the mean is a good measure however when it is not symmetric the median is better because the median is not as impacted by outliers and skewness.
...
According to the above analysis, LGA is the best on the on time departure percentage.
...
```{r speed-distance}
# scatterplot avg_speed v distance
...
Generally as average speed increases distance increases.I can't say that the best representation is linear, but the case that speed and distance are positively correlated.
...
```{r plot-to-replicate, fig.show="asis", fig.width=7, fig.height=4}
dl_aa_ua <- nycflights %>%
  filter(carrier == "AA" | carrier == "DL" | carrier == "UA")
ggplot(data = dl_aa_ua, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()
```
The code for the plot was already in the template file, I just deleted `echo = false`. The maximum cutoff point for a departure delay where it is still possible make it to your destination on American Airlines, Delta Airlines, or United Airlines on time is about 50 minutes. Although to go a step further you can get all the flights that arrived on time (less than 5 min late) and find their median and mean departure delays.

```{r dep-delay-arr-on-time}
on_time <- nycflights %>%
  filter(carrier == "AA" |
           carrier == 'DL' |
           carrier == 'UA' ,
...
```
Based on this, departing early by 1-3 minutes ensures on time arrival most of the time.